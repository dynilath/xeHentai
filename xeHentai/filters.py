...
def flt_metadata(r: HttpRequestResult) -> Dict[str, Any]:
    ...
    if r.response.status_code == 404:
        raise GalleryRemovedException(r.final_url)
    if re.match("Gallery not found", r.response.text):
        raise GalleryNotFoundException(r.final_url)
    if re.match("This gallery is pining for the fjords", r.response.text):
        raise VisibleOnlyInExhentaiException(r.final_url)
    ...
    try:
        ...
        meta["total"] = int(
            re.findall('Length:</td><td class="gdt2">(\\d+)\\s+page', r.response.text)[
                0
            ]
        )
        meta["finished"] = 0
        ...
        _ = re.findall(
            "Showing (\\d+) \\- (\\d+) of ([\\d,]+) images", r.response.text
        )[0]
...
def flt_pageurl(r: HttpRequestResult) -> List[Tuple[str, str, str]]:
    ...
    picpage: list[tuple[str, str, str]] = re.findall(
        '<a href="(%s\\/.\\/[a-f0-9]{10}\\/\\d+\\-\\d+)"><div title="Page (\\d+): ([^"]*)"'
        % RESTR_SITE,
        r.response.text,
    )
    # (page url, page id, original file name)
    if not picpage:
        raise GalleryDetailPageParseException(
            r.final_url, "can't find image page urls in gallery page"
        )
    return picpage
    ...
    def _(r: HttpRequestResult, *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if r.response.status_code == 403:
            ...
            raise QuotaExceededException(
                r.final_url, "HTTP 509 bandwidth limit exceeded"
            )
            ...
            r.content_length < 200
            and content_type.startswith("text")
            and re.search("exceeded your image viewing limits", r.response.text)
        ):
            raise QuotaExceededException(
