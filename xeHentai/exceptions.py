...
class DownloadLengthMismatchException(DownloadException):
    ...
    def __init__(self, url, expected_length, actual_length):
        message = "downloaded file length mismatch: %s, expected: %d, actual: %d" % (url, expected_length, actual_length)
        DownloadException.__init__(self, url, message)
...
def map_exception_policy(stage: str,
                         ex: Exception,
                         ignored_errors: Optional[Iterable[int]] = None) -> ExceptionPolicy:
