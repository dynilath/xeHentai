...
XEH_OPT_log_level_console = "设置控制台日志等级（DEBUG/INFO/WARNING/ERROR/CRITICAL，当前: %(default)s)"
XEH_OPT_log_level_file = "设置文件日志等级（DEBUG/INFO/WARNING/ERROR/CRITICAL，当前: %(default)s)"
...
TASK_TITLE = "任务 #{guid}(gid={gid}) 标题 {title}"
TASK_WILL_DOWNLOAD_CNT = "任务 #{guid}(gid={gid}) 将下载 {count}/{total} 个文件"
TASK_START = "任务 #{guid}(gid={gid}) 开始"
TASK_FINISHED = "任务 #{guid}(gid={gid}) 下载完成"
TASK_START_MAKE_ARCHIVE = "任务 #{guid}(gid={gid}) 开始打包"
TASK_MAKE_ARCHIVE_FINISHED = "任务 #{guid}(gid={gid}) 打包完成，保存在: {path}, 用时{time:.1f}秒"
TASK_STOP_QUOTA_EXCEEDED = "任务 #{guid}(gid={gid}) 配额超限"
...
XEH_FILE_DOWNLOADED = "#{guid} 已下载图片 #{fid} {fname}"
XEH_RENAME_HAS_ERRORS = "部分图片重命名失败:\n%s"
XEH_DOWNLOAD_HAS_ERROR = "#%s 下载图片时出错: %s, 将在稍后重试链接 %s"
...
DF_STATE_START_SCAN_PAGE = "任务 #{guid}(gid={gid}) 开始扫描页面"
DF_FILE_DOWNLOADED_SKIPPED = "#{guid} 跳过下载图片 #{fid} {reason}"
