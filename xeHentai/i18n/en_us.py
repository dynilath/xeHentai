...
XEH_OPT_log_level_console = "set console log level (DEBUG/INFO/WARNING/ERROR/CRITICAL, current: %(default)s)"
XEH_OPT_log_level_file = "set file log level (DEBUG/INFO/WARNING/ERROR/CRITICAL, current: %(default)s)"
...
TASK_TITLE = "task #{guid}(gid={gid}) title {title}"
TASK_WILL_DOWNLOAD_CNT = "task #{guid}(gid={gid}) will download {count}/{total} files"
TASK_START = "task #{guid}(gid={gid}) start"
TASK_FINISHED = "task #{guid}(gid={gid}) download finishd"
TASK_START_MAKE_ARCHIVE = "task #{guid}(gid={gid}) start making archive"
TASK_MAKE_ARCHIVE_FINISHED = "task #{guid}(gid={gid}) archive saved at: {path}, use {time:.1f}s"
TASK_STOP_QUOTA_EXCEEDED = "task #{guid}(gid={gid}) quota exceeded"
...
XEH_FILE_DOWNLOADED = "#{guid} file downloaded #{fid} {fname}"
...
DF_FULLY_MATCHED_UP_TO_DATE = "task #%s archive is up to date, no need to update, at %s"
...
DF_STATE_START_SCAN_PAGE = "task #{guid}(gid={gid}) start scanning pages"
DF_FILE_DOWNLOADED_SKIPPED = "#{guid} skipped downloading image #{fid} {reason}"
