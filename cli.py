...
from xeHentai.i18n import i18n
...
from xeHentai.util import logger

from xeHentai import config as default_config
...
def parse_opt():
    ...
    parser = argparse.ArgumentParser(
        description=i18n.XEH_OPT_DESC, epilog=i18n.XEH_OPT_EPILOG, add_help=False
    )
    ...
    parser.add_argument(
        "--daemon", action="store_true", default=_def["daemon"], help=i18n.XEH_OPT_daemon
    )
    parser.add_argument(
        "-d", "--dir", default=os.path.abspath(_def["dir"]), help=i18n.XEH_OPT_d
    )
    parser.add_argument(
        "-p", "--proxy", action="append", default=_def["proxy"], help=i18n.XEH_OPT_p
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--proxy-image",
        action="store_true",
        default=_def["proxy_image"],
        help=i18n.XEH_OPT_proxy_image,
    )
    group.add_argument(
        "--proxy-image-only",
        action="store_true",
        default=_def["proxy_image_only"],
        help=i18n.XEH_OPT_proxy_image_only,
    )
    parser.add_argument(
        "--webui-host", metavar="ADDR", default=_def["webui_host"], help=i18n.XEH_OPT_webui_host
    )
    parser.add_argument(
        "--webui-port",
        type=int,
        metavar="PORT",
        default=_def["webui_port"],
        help=i18n.XEH_OPT_webui_port,
    )
    parser.add_argument(
        "-l",
        "--logpath",
        metavar="/path/to/eh.log",
        default=os.path.abspath(_def["log_path"]),
        help=i18n.XEH_OPT_l,
    )
    parser.add_argument(
        "--log-level-console",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"{SCRIPT_NAME} {__version__} {'_dev' if DEVELOPMENT else ''}",
